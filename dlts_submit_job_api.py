...
def post_dlts_job():
	...
	jobParams = {
		"cmd" : "sudo chown -R $USER /NativeAds_Horizon && sudo chown -R $USER $HADOOP_CONF_DIR && cp /data/hadoop-conf/core-site.xml $HADOOP_CONF_DIR && mkdir horizon_check && cd horizon_check && /NativeAds_Horizon/scripts/run_timeline.sh %s %s && sleep infinity" % (
